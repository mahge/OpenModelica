...
algorithm
  (success,fileName) := matchcontinue code
    ...
    case SimCode.SIMCODE(modelInfo=mi as SimCode.MODELINFO(vars=vars))
      ...
      equation
        Error.addInternalError("SerializeTaskSystemInfo.serialize failed", sourceInfo());
      then (false,"");
  end matchcontinue;
end serializeWork;
...
algorithm
  _ := matchcontinue vars
    ...
    case SimCodeVar.SIMVARS()
      ...
      equation
        Error.addMessage(Error.INTERNAL_ERROR, {"SerializeTaskSystemInfo.serializeVars failed"});
      then fail();
  end matchcontinue;
end serializeVars;
...
algorithm
  ok := match var
    ...
    case SimCodeVar.SIMVAR()
      ...
      equation
        Error.addMessage(Error.INTERNAL_ERROR, {"SerializeTaskSystemInfo.serializeVar failed"});
      then false;
  end match;
end serializeVar;
